...
public class Dec15 extends Puzzle
{
	...
	public void dijkstra()
	{
		...
		if(currNode.name.equals("[" + endPosY + "][" + endPosX + "]"))
		{
			erg = currNode.getGesamtDistanz();
		}
		else
		{
			int[][] neighbours = getDirectAdjacents(currNode.yPos, currNode.xPos, "intTable");
			...
			for(int[] p: neighbours) // p hat [x][y]
			{
				...
				if(pos != -1) //falls zum nachbar-knoten noch nicht der kleinste weg gefunden wurde
				{
					...
					long newGesDis = currNode.getGesamtDistanz() + inputIntTable[p[1]][p[0]];
